...
def upload_products(cursor, filename):
    ...
    for product in products:
        cursor.execute(
            "INSERT OR IGNORE INTO products (id, title, short_description, avg_rating, rating_count, regular_price, sale_price, category_name)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?) ",
            (
                product.get("id", None),
...
def upload_reviews(cursor, filename):
    ...
    for review in reviews:
        cursor.execute(
            "INSERT OR IGNORE INTO reviews (id, rating, title, content, date, reviewer_name, reviewer_location, verified_purchase, product_id)"
            " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ",
            (
                review.get("id", None),
...
def upload_products_and_reviews(products_filename, reviews_filename):
    ...
    print("Uploaded products to SQL Database")
    upload_reviews(cursor, reviews_filename)
    print("Uploaded reviews to SQL Database")
    con.commit()
    con.close()
